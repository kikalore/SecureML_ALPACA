...
//
//******************************************************************************
static void incrementByOne(uint8_t *v, uint8_t n)
{
    int16_t i;
    uint8_t c;
    uint8_t x;
...
static void setCipherKey(const uint8_t *cipherKey)
{
    ...
    for(i = 0; i < CTR_DRBG_KEYLEN; i = i + 2) {
        sCipherKey = (uint8_t)(cipherKey[i]);
        sCipherKey = sCipherKey | ((uint8_t)(cipherKey[i + 1]) << 8);
        AESAKEY = sCipherKey;
    }
...
static void encryptData(const uint8_t *data, uint8_t *encryptedData)
{
    ...
    for(i = 0; i < 16; i = i + 2) {
        tempVariable = (uint8_t)(data[i]);
        tempVariable = tempVariable | ((uint8_t)(data[i + 1]) << 8);
...
//
//******************************************************************************
static uint8_t ctr_drbg_generate(uint8_t *dst, uint8_t length)
{
    uint8_t i;
    uint8_t numEncLoops;
...
//
//******************************************************************************
uint8_t rng_generateBytes(uint8_t *dst, uint8_t length)
{
